...
**AI-Powered EV Battery Telemetry Security & Intrusion Detection Platform**

AegisEV is a production-grade Electric Vehicle (EV) cybersecurity platform designed for automotive Security Operations Center (SOC) teams. It correlates real-time CAN bus telemetry, physics-based battery degradation metrics, and network intrusion data to detect, explain, and mitigate cyber-physical attacks on EV infrastructure.
...
* **Advanced 96-Cell Pack Physics Engine**
  Simulates realistic EV battery behavior, tracking state-of-health (SOH), charge cycles, localized voltage imbalances, and thermal drifts across a 96-cell topology.
* **AI-Driven Incident Correlation**
  A sliding-window event buffer that intelligently groups related network threats and physical anomalies (e.g., *CAN spoofing followed by a sudden temperature spike*) into unified, actionable security incidents.
...
* **Frontend**: Next.js (React), Tailwind CSS, Recharts, Lucide Icons, WebSockets
* **Backend**: FastAPI (Python), Motor (Async MongoDB Driver), Uvicorn
* **AI & Detection**: Scikit-Learn (Isolation Forest), Pandas, NumPy
...
1. Clone the repository:
   ```bash
   git clone https://github.com/yourusername/aegisev.git
   cd aegisev
   ```
...
1. **CAN Flood**: Overwhelms the vehicle's network bus, forcing packet drops and triggering the IDS.
2. **Replay Attack**: Captures and replays valid packets to manipulate the Battery Management System (BMS).
